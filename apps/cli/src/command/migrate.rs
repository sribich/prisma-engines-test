...
use schema_core::{
    ExtensionTypeConfig, GenericApi,
    json_rpc::types::{
        DatasourceParam, DiagnoseMigrationHistoryInput, EnsureConnectionValidityParams, MigrationList,
        MigrationLockfile, SchemaFilter, UrlContainer,
    },
    state::EngineState,
};
use tracing::debug;
    ...
    let result = state
        .ensure_connection_validity(EnsureConnectionValidityParams {
            datasource: DatasourceParam::ConnectionString(UrlContainer {
                url: config_context.context.inner.configuration.datasources[0]
                    .get_connection_url()
                    .clone()
                    .value
                    .unwrap()
